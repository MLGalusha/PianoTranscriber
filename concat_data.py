...
def save_dataframes(file_list, batch_size=50, batch_number_start=1):
    dead_files = []
    df_list = []
    ...
    for path in file_list:
        ...
        if len(df_list) >= batch_size or iterations == total_files:
            ...
            try:
                ...
                dead_files.extend([df_path for df in df_list for df_path in df['file_path'].unique()])
            finally:
                # Clear memory
                del df_list[:]
                del batch_df
...
for year in years:
    ...
    dead_files, batch_number = save_dataframes(file_paths, batch_size=50, batch_number_start=batch_number)
    ...
    while dead_files and attempt < max_attempts:
        ...
        dead_files, batch_number = save_dataframes(dead_files, batch_size=50, batch_number_start=batch_number)

    if dead_files:
        print(f"\nFailed to process the following files in Year {year} after {max_attempts} attempts:\n{dead_files}\n")

    # Delete the year directory to free up disk space
    try:
        # Use shutil.rmtree to delete the directory permanently
        shutil.rmtree(year_dir)
        print(f"Deleted directory {year_dir} to free up disk space.")
    except Exception as e:
        print(f"Error deleting directory {year_dir}: {e}")
